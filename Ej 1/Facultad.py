...
class Facultad():
	...
	def AgregarCarrera(self,fila):
		c = Carrera(int(fila[1]),fila[2],fila[3],fila[4],fila[5])
		self.__carrera.append(c)

	def MostrarDatos(self):
		print("Facultad: ",self.__nombre)
		for i in range(len(self.__carrera)):
			print(f'Nombre de la carrera: {self.__carrera[i].getNom()}')
			print(f'Duracion de la carrera: {self.__carrera[i].getDuracion()}')